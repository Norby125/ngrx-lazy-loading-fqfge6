...
@Component({
  ...
  template: "<div style='display: flex; flex-direction: row;'><button (click)='inc()'>+</button><h1>{{(lazyStateCounter$ | async)}}</h1><button (click)='dec()'>-</button></div>"
})
export class LazyLoadedComponent {
  ...
  constructor(private store: Store<fromLazyStore.LazyState>) {
    this.lazyStateCounter$ = this.store.pipe(select(fromLazyStore.getMyCounter))
  }
  inc() {
